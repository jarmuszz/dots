...
do
  ...
  local on_attach
  local function _12_(client, bufn)
    _G.assert((nil ~= bufn), "Missing argument bufn on unknown:87")
    _G.assert((nil ~= client), "Missing argument client on unknown:87")
...
do
  ...
  hydra({name = "Spell", hint = hint, config = {invoke_on_body = true, hint = {border = "rounded", position = "middle"}}, mode = "n", body = "<M-s>", heads = {{"s", _13_}, {"p", _14_}, {"e", _15_}, {"d", _16_}}})
end
vim.g.mapleader = " "
